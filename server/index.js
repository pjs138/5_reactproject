...
// 3) db.query를 사용하여 mysql데이터베이스에 쿼리를 실행합니다
    db.query(sqlQuery,(err, result) => {
// 4) 쿼리 실행후에 에러가 발생하면 적절히 처리하거나 결과를 클라이언트에게 보낸다.
    res.send(result)
})
})
...
app.post("/insert",(req, res) => {
    const title = req.body.title; //새로운 글을 쓰는데 숫자가 자동으로 1 증가하면서 타이틀과 내용을 mysql에서 레코드한다
    const content = req.body.title;

    const sqlQuery = 
    "INSERT INTO BOARD (BOARD_TITLE, BOARD_CONTENT, REGISTER_ID) VALUES (?, ?, '박준상')"
